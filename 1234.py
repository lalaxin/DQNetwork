...
# 用户需求,每个时间段用户需求随机到来（随机初始化用户数量）以及用户的起始区域，目的地，用户实际到达的区域
def init_user_demand():
    userdemand=[[[0]for i in range (usernum)] for t in range (T)]
    for t in range (T):
        for i in range (usernum):
            userdemand[t][i]=[random.randint(0,regionnum-1),random.randint(0,regionnum-1),0]
    return userdemand
...
N_ACTIONS = int(math.pow(9,usernum))
# 接收的observation维度
...
def run_this():
    ...
    for i_episode in range(400):
        ...
        for t in range (T):
            ...
            if (t == 0):
                ...
                for i in range (usernum):
                    if(s_1[user[t][i][0]]!=0):
                        ...
                        # 将该区域的临近区域表示出来，注意边界区域,user内存储的都是区域
                        if(user[t][i][0]<xcell):
                            temp.remove(user[t][i][0]-xcell-1)
                            temp.remove(user[t][i][0]-xcell)
                            temp.remove(user[t][i][0]-xcell+1)
                        if(user[t][i][0]%xcell==0):
                            temp.remove(user[t][i][0]-1)
                            temp.remove(user[t][i][0]+xcell-1)
                            if (user[t][i][0]-xcell-1) in temp:
                                temp.remove(user[t][i][0]-xcell-1)
                        if(region[user[t][i][0]][1]//celllength==xcell-1):
                            temp.remove(user[t][i][0]+1)
                            temp.remove(user[t][i][0]+xcell+1)
                            if (user[t][i][0]-xcell+1) in temp:
                                temp.remove(user[t][i][0]-xcell+1)
                        if (region[user[t][i][0]][2] % celllength == ycell - 1):
                            temp.remove(user[t][i][0]+xcell)
                            if (user[t][i][0]+xcell-1) in temp:
                                temp.remove(user[t][i][0]+xcell-1)
                            if (user[t][i][0]+xcell+1) in temp:
            ...
            # dqn.choose_action()输出的a是一个动作的值，然后根据这个值判断是哪个动作
            # 一共有9的usernum次方，将每一个值分配给对应的用户动作
            ...
            for i in range(usernum):
                # 将相邻的区域与1，2，3，4，5，6，7，8，0(如何处理边界区域)，还未处理
                tempp = [user[t][i][1] + xcell + 1,user[t][i][1] - xcell - 1, user[t][i][1] - xcell, user[t][i][1] - xcell + 1, user[t][i][1] - 1,user[t][i][1],user[t][i][1] + 1, user[t][i][1] + xcell - 1, user[t][i][1] + xcell]
                # 处理边界点
                if (user[t][i][1] < xcell):
                    tempp[1]=-1
                    tempp[2]=-1
                    tempp[3]=-1
                if (user[t][i][1] % xcell == 0):
                    tempp[4]=-1
                    tempp[7]=-1
                    if (user[t][i][1] - xcell - 1) in tempp:
                        tempp[1]=-1
                if (region[user[t][i][1]][1] // celllength == xcell - 1):
                    tempp[6]=-1
                    tempp[0]=-1
                    if (user[t][i][1] - xcell + 1) in tempp:
                        tempp[3]=-1
                if (region[user[t][i][1]][2]// celllength == ycell - 1):
                    tempp[8]=-1
                    if (user[t][i][1] + xcell - 1) in tempp:
                        tempp[7]=-1
                    if (user[t][i][1] + xcell + 1) in tempp:
                ...
                if(i==usernum-1):
                    if(tempp[int((a+1)%9)]==-1):
                        user[t][i][2]==user[t][i][1]
                    else:
                        user[t][i][2]=tempp[int((a+1)%9)]
                else:
                    if (tempp[int((a+1)//math.pow(9,usernum-i-1))]== -1):
                        user[t][i][2] == user[t][i][1]
                    else:
                        user[t][i][2] = tempp[int((a+1)//math.pow(9,usernum-i-1))]
