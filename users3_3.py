"""
时间间隔十分钟，8月15号12点到14点的数据,存于data4_12-14.xlsx中
"""
...
class getuser():
    def getusers(self):
        ...
        excel = xlrd.open_workbook("./data4_12-14.xlsx")

        # 获取excel的sheet表
        sheet = excel.sheet_by_name("Sheet1")
        starttime = datetime.datetime(2016,8,15,12,00,00)
        endtime = datetime.datetime(2016,8,15,12,10,00)

        # 统计截至日期
        deadday = datetime.datetime(2016, 8, 15, 14, 00, 00)
        onetime = []
        one = []
        user = []
        print("-------start----")
...
def getregion():
    ...
    # 写入excel文件
    writer = pd.ExcelWriter("./userregion12-14.xlsx")
    data.to_excel(writer, 'sheet1', float_format='%.5f')
    writer.save()
    writer.close()
