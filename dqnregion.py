...
def run_this():
    ...
    # 判断用户的目的地在哪个区域进行区域间的匹配
    for t in range (T):
        for i in range(len(init_user[t])):
            if (init_user[t][i][2] == cell * celllength and init_user[t][i][3] == cell * celllength):
                tempa = int(cell * cell - 1)
            elif (init_user[t][i][2] == cell * celllength):
                tempa = int(init_user[t][i][3] / celllength) * cell + int(init_user[t][i][2] / celllength) - 1
            elif (init_user[t][i][3] == cell * celllength):
                tempa = int(init_user[t][i][3] / celllength) * cell + int(init_user[t][i][2] / celllength) - cell
            else:
                tempa = int(init_user[t][i][3] / celllength) * cell + int(init_user[t][i][2] / celllength)
            if (tempa < cell * cell):
                init_user[t][i][2]=init_region[tempa][3]
                init_user[t][i][3]=init_region[tempa][4]
    init_s=init_state()
