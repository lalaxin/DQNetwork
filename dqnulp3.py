"""
匹配时不使用最大距离进行匹配,使用原始方案进行匹配以及调度，同时可在附近区域取车
"""
import torch
                    ...
                    if (tempa < cell * cell):
                        ...
                        if (t != 0):
                            if (preregion[tempa][1] <= 0):
                                ...
                                preregion[tempa][1] -= 1
                neighborregion = [-1, cell, 1, -cell, cell - 1, cell + 1, -cell - 1, -cell + 1, -2, 2 * cell, +2,
                                  -2 * cell]
                    ...
                    for i in range(len(removeuser)):
                        for j, neighbor in enumerate(neighborregion):
                            ...
                            if (j == 0):
                                ...
                                if (removeuserreggion[2 * i + 1] + cell > 0 or (
                                removeuserreggion[2 * i + 1]) % cell == 0):
                                    continue
                            elif (j == 6):
                                if (removeuserreggion[2 * i + 1] - cell < 0 or (
                                removeuserreggion[2 * i + 1]) % cell == 0):
                                ...
                                if (removeuserreggion[2 * i + 1] + cell > 0 or (
                                removeuserreggion[2 * i + 1]) % cell == cell - 1):
                                    continue
                            elif (j == 6):
                                if (removeuserreggion[2 * i + 1] - cell < 0 or (
                                removeuserreggion[2 * i + 1]) % cell == cell - 1):
                            ...
                            dneighbor = math.sqrt(
                                math.pow(removeuser[i][2] - region[removeuserreggion[2 * i + 1] + neighbor][3],
                                         2) + math.pow(
                                    removeuser[i][3] - region[removeuserreggion[2 * i + 1] + neighbor][4], 2))
                            ...
                            if (dneighbor <= removeuser[i][4] and s_[removeuserreggion[2 * i + 1] + neighbor] > 0):
                                ...
                                user[t][removeuserreggion[2 * i]][0] = region[removeuserreggion[2 * i + 1] + neighbor][
                                    3]
                                user[t][removeuserreggion[2 * i]][1] = region[removeuserreggion[2 * i + 1] + neighbor][
                                    4]
